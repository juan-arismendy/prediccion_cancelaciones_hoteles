#!/usr/bin/env python3
"""
Sistema de Persistencia de Modelos para Predicción de Cancelaciones Hoteleras
Permite guardar y cargar modelos entrenados, resultados y métricas para evitar reentrenamiento.
"""
...
def train_and_save_models(X, y, session_name=None):
    """
    Entrena todos los modelos y los guarda con persistencia
    ...
    for model_name, config in models_config.items():
        ...
        try:
            ...
            grid_search = GridSearchCV(
                ...
                cv=cv,
                scoring='f1',
                n_jobs=2,  # Reducido de -1 a 2 para evitar problemas de memoria
                verbose=1
            )
    ...
    print(f"\n🎉 Entrenamiento completado. Sesión guardada: {session_name}")
...
def load_trained_models():
    """
    Carga los modelos entrenados y sus resultados desde la sesión más reciente.
    
    Returns:
        tuple: (models_dict, results_dict, session_info)
    """
    try:
        # Buscar la sesión más reciente
        base_path = os.path.join(os.path.dirname(__file__), '..', '..', 'saved_models')
        sessions_dir = os.path.join(base_path, 'metadata')
        ...
        session_name = latest_session.replace('.json', '')
        
        print(f"📂 Cargando sesión más reciente: {session_name}")
        
        # Cargar información de la sesión
        session_file = os.path.join(sessions_dir, latest_session)
        with open(session_file, 'r') as f:
            session_info = json.load(f)
        ...
        for model_name in session_info['models']:
            model_file = os.path.join(models_dir, f"{model_name}_{session_name}.pkl")
        ...
        for model_name in session_info['models']:
            results_file = os.path.join(results_dir, f"{model_name}_results_{session_name}.json")
            if os.path.exists(results_file):
                with open(results_file, 'r') as f:
                    results_dict[model_name] = json.load(f)
                print(f"✅ Resultados de {model_name} cargados desde: {results_file}")
            else:
                print(f"⚠️  No se encontraron resultados para {model_name}")
...
if __name__ == "__main__":
    print("🔧 Sistema de Persistencia de Modelos - Predicción de Cancelaciones Hoteleras")
    ...
    if available_sessions:
        ...
        print("📁 No se encontraron sesiones guardadas")
        print("💡 Para entrenar nuevos modelos, usa: train_and_save_models(X, y)")
    
    print("\n📋 Funciones disponibles:")
    print("- train_and_save_models(X, y, session_name=None): Entrena y guarda todos los modelos")
