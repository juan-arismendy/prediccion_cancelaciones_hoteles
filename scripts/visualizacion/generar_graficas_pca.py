#!/usr/bin/env python3
"""
Script para generar gráficas específicas de PCA
Basado en los archivos de referencia seleccion_secuencial_final.py y extraccion_caracteristicas_pca_final.py
"""
...
def analyze_pca_components(X, y, preprocessor):
    ...
    if np.any(np.isnan(X_preprocessed)) or np.any(np.isinf(X_preprocessed)):
        print("Advertencia: Se encontraron valores NaN o infinitos. Limpiando datos...")
        X_preprocessed = np.nan_to_num(X_preprocessed, nan=0.0, posinf=0.0, neginf=0.0)
    
    # Verificar que la matriz no es singular
    if X_preprocessed.shape[1] > X_preprocessed.shape[0]:
        print("Advertencia: Más características que muestras. Reduciendo dimensionalidad...")
        n_components_max = min(X_preprocessed.shape[0] - 1, X_preprocessed.shape[1] - 1)
...
def main():
    ...
    # 3. Analizar componentes principales
    ...
    # 5. Evaluación con características originales
    print("\n" + "="*60)
    ...
    print(f"\n✅ Análisis PCA completado. Gráficas guardadas:")
    print(f"  - pca_analysis.png")
    print(f"  - pca_comparison_table.png")
    print(f"  - pca_reduction_results.png")
    print(f"  - pca_metrics_comparison.png")
