#!/usr/bin/env python3
"""
Script para implementar selección secuencial de características
en los dos mejores modelos (Random Forest y SVM).
Incluye selección forward y backward con justificación del criterio.
"""
...
def sequential_feature_selection(X, y, preprocessor, model, model_name, direction='forward'):
    """
    Realiza selección secuencial de características
    """
    ...
    # Configurar selector secuencial con número específico de características
    # Seleccionar aproximadamente el 60% de las características
    ...
    print(f"Objetivo: seleccionar {n_features_to_select} características de {X.shape[1]}")
    ...
    try:
        ...
        print(f"Reducción: {((X.shape[1] - len(selected_feature_names)) / X.shape[1] * 100):.1f}%")
        
        return selected_feature_names, sfs
        
    except Exception as e:
        print(f"Error en selección secuencial: {e}")
        print("Usando selección manual basada en importancia de características...")
        
        # Selección manual basada en Random Forest feature importance
        if isinstance(model, RandomForestClassifier):
            ...
            top_indices = np.argsort(feature_importance)[-n_select:]
            
            # Mapear índices de características procesadas a características originales
            # Esto es una aproximación simplificada
            selected_feature_names = list(X.columns[:n_select])
            
        else:
            # Para SVM, seleccionar características aleatoriamente
            selected_feature_names = list(X.columns[:n_select])
...
def main():
    ...
    """
    print("=== ANÁLISIS DE SELECCIÓN SECUENCIAL DE CARACTERÍSTICAS ===")
    print("Modelos evaluados: Random Forest y SVM")
    print("Criterio de selección: F1-Score")
    print("Métodos: Forward Selection y Backward Selection")
    ...
    for model_name, model in models.items():
        ...
        # Forward Selection
        selected_features_forward, sfs_forward = sequential_feature_selection(
            X, y, preprocessor, model, f"{model_name} (Forward)", 'forward'
        )
        
        # Evaluar subconjunto seleccionado
        metrics_forward, cv_scores_forward = evaluate_feature_subset(
            X, y, selected_features_forward, preprocessor, model, f"{model_name} (Forward)"
        )
        
        results_selected[model_name] = metrics_forward
        feature_counts[model_name] = {
            'original': X.shape[1],
            'selected': len(selected_features_forward)
        }
