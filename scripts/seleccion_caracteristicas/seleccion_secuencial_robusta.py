#!/usr/bin/env python3
"""
Script robusto para selección secuencial de características
...
def select_features_robust(X, y, numerical_features, categorical_features, model_name):
    """
    Selección robusta de características basada en importancia/correlación
    """
...
def evaluate_feature_subset_robust(X, y, selected_features, preprocessor, model, model_name):
    """
    Evalúa el subconjunto de características seleccionado de forma robusta
    """
    ...
    try:
        ...
        print(f"F1-Score CV (5-fold): {cv_mean:.3f} ± {cv_std*2:.3f}")
    ...
    return metrics, cv_scores if 'cv_scores' in locals() else np.array([0.0])
...
def main():
    ...
    for model_name, model in models.items():
        ...
        selected_features = select_features_robust(X, y, numerical_features, categorical_features, model_name)
        
        # Evaluar subconjunto seleccionado
        metrics_selected, cv_scores_selected = evaluate_feature_subset_robust(
