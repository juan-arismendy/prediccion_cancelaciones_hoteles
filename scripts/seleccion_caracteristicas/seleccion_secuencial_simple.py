#!/usr/bin/env python3
"""
Script simplificado para selección secuencial de características
...
def select_features_by_importance(X, y, preprocessor, model, model_name):
    """
    Selecciona características basándose en importancia para Random Forest
    o correlación para SVM
    """
    ...
    if isinstance(model, RandomForestClassifier):
        ...
        # Mapear características procesadas a originales
        for i, feature in enumerate(X.columns):
            if feature in numerical_features:
                feature_names_processed.append(feature)
            else:
                # Para características categóricas, agregar múltiples columnas
                unique_values = X[feature].nunique()
                for j in range(unique_values):
                    feature_names_processed.append(f"{feature}_{j}")
        
        # Seleccionar características con mayor importancia
        importance = rf_model.feature_importances_
        n_select = max(1, int(len(importance) * 0.6))  # Seleccionar 60%
        
        # Obtener índices de características más importantes
        top_indices = np.argsort(importance)[-n_select:]
        
        # Mapear de vuelta a características originales
        selected_features = []
        for idx in top_indices:
            if idx < len(X.columns):
                selected_features.append(X.columns[idx])
        
        print(f"Características seleccionadas por importancia: {len(selected_features)}")
        
    else:
        # Para SVM, usar correlación con el target
        ...
        # Agregar algunas características categóricas importantes
        categorical_important = ['hotel', 'market_segment', 'deposit_type', 'customer_type']
        for cat_feature in categorical_important:
            if cat_feature in X.columns:
                selected_features.append(cat_feature)
        
        print(f"Características seleccionadas por correlación: {len(selected_features)}")
...
def main():
    ...
    for model_name, model in models.items():
        ...
        selected_features = select_features_by_importance(X, y, preprocessor, model, model_name)
